...
/*void Control::Mark_Active_By_Person(std::vector<Person*>& persons, size_t size_persons)
{
	person_selected = persons[0];
}*/

void Control::Clear_Person_Selected()
{
...
void Control::Draw_Walk_Area(Camera& camera, Battle_Map& battle_map, std::vector <Person*>& persons)
{
		...
		for (int i = -person_selected->walk_range; i <= person_selected->walk_range; i++)
			for (int j = -person_selected->walk_range; j <= person_selected->walk_range; j++)
			{
				...
				if (person_selected->x + i >= 0 && person_selected->x + i <= battle_map.map_w - 1 && person_selected->y - j >= -(battle_map.map_h - 1) && person_selected->y - j <= 0)
					...
					if (battle_map.No_Way_Object(person_selected->x + i, -person_selected->y + j, persons))
...
void Control::Draw_Person_Way_Walk(Camera& camera, Battle_Map& battle_map, std::vector <Person*>& persons)const
{
	if (person_selected)
	{
		for (int i = 0; i < person_selected->walk_range; i++)
			...
			if (person_selected->step[i].x >= 0 && person_selected->step[i].x <= battle_map.map_w - 1 && 
				...
				//Not draw unwalk tiles
				if (battle_map.No_Way_Object(person_selected->step[i].x, -person_selected->step[i].y, persons))
					//Draw way tile
					battle_map.map[int(-person_selected->step[i].y)][int(person_selected->step[i].x)]->Draw(camera, blue);
	}
}

void Control::Save_Walk_Coords_to_Arr(GLFWwindow* window, Camera& camera, Battle_Map& battle_map, std::vector <Person*>& persons)
{
	...
	if (person_selected && battle_interface->go_flag && !person_selected->walk_flag)
	{
		...
		case 2: {
			...
			{
				//Don't save way tile to arr AND don't LMB for walk if NO WAY TILE
				if (battle_map.No_Way_Object(x_over, -y_over, persons))
				{
		...
		case 3: {
			...
			if (person_selected->step[0].x >= 0 && person_selected->step[0].y <= 0)
			{
					...
					y_over >= person_selected->step[0].y - 1 && y_over <= person_selected->step[0].y + 1)
				{
					...
					//Clear other cells, NO first
					for (int i = 2; i < person_selected->walk_range; i++)
					{
						person_selected->step[i].x = -1.0f;
						person_selected->step[i].y = 1.0f;
					}
