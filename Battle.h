...
public:
	...
	//When phase ends switch to the next
	//void Switch_to_Next_Phase(std::vector<Person*>& persons);

	//Func activate next step id current person or next person
	void Mark_Active_By_Person(std::vector<Person*>& persons);

	//Func save World coords of click, it moved from Control. May be it must rebuild
	//void Click_Lmb(GLFWwindow* window, Camera& camera, std::vector<Person*>& persons, size_t size_persons);
